...
if __name__ == "__main__":
    ...
    signal_f = fundamental.analyze("000001", market_data)
    print(f"基本面: {signal_f.action.value} (信心 {signal_f.confidence:.2f})")
    print(f"  理由: {signal_f.reasoning}")
    
    signal_t = technical.analyze("000001", market_data)
    print(f"技术面: {signal_t.action.value} (信心 {signal_t.confidence:.2f})")
    print(f"  理由: {signal_t.reasoning}")
    
    signal_s = sentiment.analyze("000001", market_data)
    print(f"情绪面: {signal_s.action.value} (信心 {signal_s.confidence:.2f})")
    print(f"  理由: {signal_s.reasoning}")
    
    # 展示工具列表
    print("\n[注册的工具]")
    for tool_info in fundamental.list_tools():
