...
if __name__ == "__main__":
    print("=" * 60)
    print("FinancialTools 测试")
    print("=" * 60)
    
    tools = FinancialTools()
    print(f"FinRobot 可用: {tools.is_finrobot_available}")
    
    # 测试新闻分析
    print("\n[新闻分析测试]")
    news = "美联储宣布加息25个基点，市场大跌3%，AAPL 跌破200日均线"
    result = tools.parse_financial_news(news)
    ...
    # 测试财报分析
    print("\n[财报分析测试]")
    earnings = tools.analyze_earnings(
        ticker="AAPL",
        quarter="Q4 2024",
        revenue=94.9e9,
        revenue_est=89.5e9,
        eps=2.10,
        eps_est=1.95,
        guidance_text="Management raised full year guidance"
    )
    print(f"  收入超预期: {earnings.revenue_surprise:.1%}")
    print(f"  EPS 超预期: {earnings.eps_surprise:.1%}")
    print(f"  指引: {earnings.guidance}")
    print(f"  洞察: {earnings.key_insights}")
    
    # 测试 CoT Prompt
    print("\n[CoT Prompt 测试]")
    market = {"price": 3000, "trend": "下跌", "panic_level": 0.7, "csad": 0.02}
    account = {"pnl_pct": -0.05, "avg_cost": 3150}
    prompt = tools.get_cot_investment_prompt(market, account)
