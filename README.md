...
- 工程与测试：`pytest`, `nest_asyncio`, `pybind11`, `setuptools`
...
- 运行 `python main.py` 报 `EOFError`
  - 原因：无交互终端下无法输入 API Key。
  - 处理：改用 `python -m streamlit run app.py` 或提前设置 `DEEPSEEK_API_KEY`。
